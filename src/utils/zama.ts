...
export class ZamaClient {
  ...
  // misconfiguration (see `classify`) is rethrown.
  ...
  // block sweep; "pending" is retried when an ACL grant lands.
  private classify(err: unknown): DecryptOutcome {
    const outcome = matchZamaError<DecryptOutcome>(err, {
      ...
      // message (the exact strings the SDK throws in relayer-cleartext.ts's
      // #assertDecryptAuthorization / #assertDelegation). Sniff those; any other
      ...
      // the sweep. The "stranger → pending" test is the canary if the SDK rewords these.
      DECRYPTION_FAILED: (e) =>
        /not authorized|not delegated|not allowed/i.test(e.message)
